        ...
        # Stato treno
            ...
            def _get_int(nome_dest):
                ...
                for ni, ii in INTERSCAMBI.items():
                    ...
                    pd = nd.split()
                    if pi and all(p in pd for p in pi[:2]):
                        return ii, ni
            ...
            def _cerca_int(sid, snome, arr_ms, t1):
                ...
                if not tb or not isinstance(tb, list):
                    print(f"[INT] {snome} nessun treno in partenza", flush=True)
                    return sols
                ...
                for t2 in tb[:20]:
                    ...
                    print(f"[INT2] {t2.get('numeroTreno')} dest={dest2} ok={ok}", flush=True)
            ...
            # Cerca cambi in sequenza (no thread — più affidabile)
            tutte = []
            for t in treni_a[:10]:
                num = t.get('numeroTreno')
